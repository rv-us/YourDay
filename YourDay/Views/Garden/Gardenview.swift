...
// Dynamic text colors - white for night mode, dark for day mode
let vibrantPrimaryTextColor = Color(UIColor { _ in 
    GardenAssetHelper.isDayTime() ? UIColor(VibrantLightTheme.primaryText) : UIColor.white
})
let vibrantSecondaryTextColor = Color(UIColor { _ in 
    GardenAssetHelper.isDayTime() ? UIColor(VibrantLightTheme.secondaryText) : UIColor.white.withAlphaComponent(0.9)
})
...
struct GardenView: View {
        ...
        NavigationView {
            GeometryReader { geometry in
                ZStack {
                    ...
                    // World layer — single camera transform keeps island & grid pixel-locked
                    ZStack {
                        islandImageView
                            .zIndex(0.5)

                        IslandGridOverlayView(
                            playerStats: playerStats,
                            isFertilizerModeActive: $isFertilizerModeActive,
                        isSellModeActive: $isSellModeActive,
                        plantFeedbackItems: $plantFeedbackItems,
                        draggedPlant: $draggedPlant,
                        isTutorialActive: isTutorialActive,
                        currentTutorialStep: currentTutorialStep,
                        onEmptyTileTap: { position in
                            self.plantingSheetItem = IdentifiableGridPositionWrapper(position: position)
                        },
                        onWaterAction: { plantIndex in
                            waterSinglePlant(at: plantIndex)
                        },
                        onSellAction: { plant in
                        ...
                        onFertilizeAction: { plant in
                            attemptToFertilize(plant: plant)
                        },
                        onTapInSellModeAction: { plant in
                        ...
                        onInfoAction: { plant in
                            if !isTutorialActive && !isSellModeActive && !isFertilizerModeActive {
                                self.selectedPlantForInfo = plant
                            }
                        },
                        onAlertAction: { title, message in
                            showStandardAlert(title: title, message: message)
                        },
                    ...
                    .frame(
                        width: GardenMapConfig.mapDimensions()?.width ?? 0,
                        height: GardenMapConfig.mapDimensions()?.height ?? 0,
                ...
                .simultaneousGesture(
                    DragGesture()
                        .onChanged { value in
                            ...
                            panOffset = clampedPanOffset(newOffset)
                        }
                        .onEnded { _ in
                            lastPanOffset = panOffset
                        }
    ...
    @ViewBuilder
    private var islandImageView: some View {
        let islandCenter = IslandGridConfig.islandCenterInMapSpace()
        let displaySize  = IslandGridConfig.islandDisplaySize
        let islandName = GardenAssetHelper.islandImageName()

        Image(islandName)
            .resizable()
            .frame(width: displaySize.width, height: displaySize.height)
            .position(x: islandCenter.x, y: islandCenter.y)
    }
    ...
    private var currentSeasonDisplay: some View {
        ...
        // gardenGridSection removed — replaced by IslandGridOverlayView in body
        ...
        /// Clamps a pan offset so the map edges never reveal empty/black areas.
        /// Uses the full screen bounds (not the content-area geometry) because the
        /// background extends into safe areas via `.edgesIgnoringSafeArea(.all)`.
        private func clampedPanOffset(_ proposed: CGSize) -> CGSize {
            guard let mapDims = GardenMapConfig.mapDimensions() else {
                return proposed
            }

            // The background covers the full screen, so clamp against screen bounds
...
struct EmptyPlotIconView: View {
    ...
    struct PlantPlotView: View {
        ...
        var body: some View {
            VStack(spacing: 10) {
                currentPlantVisual()
                    .frame(minHeight: 400, maxHeight: 600).aspectRatio(1, contentMode: .fit).cornerRadius(40).padding(.top, 10)
                    .overlay( Group {
                        if isFertilizerModeActive && !plant.isFullyGrown {
                            RoundedRectangle(cornerRadius: 40).stroke(vibrantFertilizerButtonColor, lineWidth: 15).opacity(0.7)
                        }
                        else if isSellModeActive && plant.isFullyGrown {
                            RoundedRectangle(cornerRadius: 40).stroke(vibrantSellButtonColor, lineWidth: 15).opacity(0.7)
                                .overlay(Image(systemName: "dollarsign.circle.fill").font(.system(size: 80)).foregroundColor(vibrantSellButtonColor.opacity(0.9)).padding(25).background(.ultraThinMaterial).clipShape(Circle()))
                        }
                    })
                    .overlay( Group { if let text = localFeedbackText { Text(text).font(.system(size: 30, weight: .bold)).foregroundColor(localFeedbackColor).padding(EdgeInsets(top: 15, leading: 30, bottom: 15, trailing: 30)).background(dynamicSecondaryBackgroundColor.opacity(0.8)).cornerRadius(25).shadow(radius: 10).offset(y: localFeedbackYOffset).opacity(localFeedbackOpacity) }})
                VStack(alignment: .center, spacing: 1) {
                    Text(plant.name).font(.system(size: 50, weight: .medium)).lineLimit(1).minimumScaleFactor(0.8)
                        .foregroundColor(vibrantPrimaryTextColor)
                    if plant.isFullyGrown {
                        Text("Grown!")
            ...
            .frame(minHeight: 500, maxHeight: 600).cornerRadius(50)
            .onTapGesture {
            ...
            .onChange(of: feedbackItem?.id) {
                if let newFeedback = feedbackItem {
                    ...
                    withAnimation(.easeOut(duration: 0.3)) { localFeedbackYOffset = -150 }
