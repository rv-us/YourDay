...
struct LastDayView: View {
    ...
    private var currentDisplayDate: Date? {
        guard !uniqueSortedDates.isEmpty, dateOffset < uniqueSortedDates.count, dateOffset >= 0 else {
            // Updated to be "today" if no summaries, or "yesterday" as a common default for a daily summary.
            // Adjust this logic if currentDisplayDate should truly be null in some cases.
    ...
    var body: some View {
        VStack(spacing: 16) {
            ...
            if !summariesForDisplayDate.isEmpty || (isModal && xpInfoForDisplayDate.xpEarnedToday == 0 && uniqueSortedDates.isEmpty) {
                Text("+\(Int(animatedPointsTotal)) Points!")
                    .font(.system(size: 40, weight: .heavy, design: .rounded))
                    .foregroundColor(plantMediumGreen) // Themed color for earned points
                    .id("animatedPointsText-\(dateOffset)")
            ...
            if isModal { /* Dismiss button */
                Button(action: { withAnimation { dismiss() } }) {
                    Text(totalPointsForDisplayDate > 0 && !summariesForDisplayDate.isEmpty ? "Awesome!" : "Close")
                        ...
                        .background(
                            totalPointsForDisplayDate > 0 && !summariesForDisplayDate.isEmpty ? plantMediumGreen : plantDustyBlue
        ...
        .padding(.vertical) // Keep this padding
        .background(plantBeige.edgesIgnoringSafeArea(.all)) // Apply themed background
        .onAppear {
            let initialPoints = totalPointsForDisplayDate
            self.animatedPointsTotal = 0
            // Added slight delay for the date to settle before animating points
            DispatchQueue.main.asyncAfter(deadline: .now() + 0.1) {
                withAnimation(.easeOut(duration: 1.0)) { self.animatedPointsTotal = initialPoints }
                updateContinueButtonVisibility(points: initialPoints, isInitialAppearance: true)
            }
        ...
        .onChange(of: currentDisplayDate) { oldDate, newDate in
            ...
            let newTotalPoints = totalPointsForDisplayDate
            self.animatedPointsTotal = 0
            // Added slight delay for date change animation to finish before points animate
            DispatchQueue.main.asyncAfter(deadline: .now() + 0.05) {
                withAnimation(.easeOut(duration: 1.0)) { self.animatedPointsTotal = newTotalPoints }
                updateContinueButtonVisibility(points: newTotalPoints)
            }
    ...
    private func updateContinueButtonVisibility(points: Double, isInitialAppearance: Bool = false) {
        if isModal {
            if points > 0 && !summariesForDisplayDate.isEmpty {
                // This logic might need slight adjustment based on exact tutorial flow
                // For now, ensure button shows if points were earned or it's not the initial 0-point state
                if !isInitialAppearance { self.showContinueButton = false } // Reset for new animation
                DispatchQueue.main.asyncAfter(deadline: .now() + (isInitialAppearance ? 0.1 : 1.0) ) { // Longer delay after switch
                    withAnimation(.spring(response: 0.4, dampingFraction: 0.6)) { self.showContinueButton = true }
                }
            } else { self.showContinueButton = true } // Show immediately if no points or no summary
        }
    }
}
...
struct TaskSummaryRow: View {
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 5) {
            HStack {
                Image(systemName: taskResult.mainTaskCompletedOnTargetDay ? "checkmark.circle.fill" : "circle.dashed")
                    .foregroundColor(taskResult.mainTaskCompletedOnTargetDay ? plantDarkGreen : plantDustyBlue) // Themed colors
            ...
            if !taskResult.subtaskPoints.isEmpty {
                ForEach(taskResult.subtaskPoints.filter { $0.earned > 0 }, id: \.title) { sub in
                    HStack {
                        ...
                        Text("+\(Int(sub.earned))")
                            .font(.subheadline)
                            .foregroundColor(plantMediumGreen.opacity(0.8)) // Themed color
                    }.padding(.leading, 25)
                }
            }
        }
        .padding()
        .background(plantPastelGreen.opacity(0.4)) // Themed background for summary rows
        .cornerRadius(10)
        .shadow(color: Color.black.opacity(0.05), radius: 3, x: 0, y: 2) // Keep subtle shadow
    }
}
...
struct TasksCompletionProgressView: View {
    ...
    var body: some View {
        ZStack {
            Circle()
                .stroke(plantPastelGreen.opacity(0.5), lineWidth: 10) // Themed background circle
            Circle()
                .trim(from: 0.0, to: progress)
                .stroke(plantMediumGreen, style: StrokeStyle(lineWidth: 10, lineCap: .round)) // Themed progress color
                .rotationEffect(.degrees(-90))
                .animation(.interpolatingSpring(stiffness: 100, damping: 10), value: progress)
            VStack {
        ...
        VStack(alignment: .leading, spacing: 8) {
            HStack {
                ...
                if showLevelUpMessage {
                    Text("LEVEL UP!")
                        .font(.headline)
                        .foregroundColor(plantPink) // Themed level up color
                        .transition(.scale.combined(with: .opacity))
                }
            }
            ...
            ProgressView(value: animatedXP, total: displayedXPForNextLevel) {
                ...
                Text("\(Int(animatedXP)) / \(Int(displayedXPForNextLevel)) XP")
                    .font(.caption)
                    .foregroundColor(plantMediumGreen) // Themed XP text color
            }
            .progressViewStyle(LinearProgressViewStyle(tint: plantMediumGreen)) // Themed progress bar color
            .background(
                RoundedRectangle(cornerRadius: 5) // Background for the whole bar
                    .fill(plantPastelGreen.opacity(0.3)) // Themed background color for the bar track
            )
            .cornerRadius(5)
